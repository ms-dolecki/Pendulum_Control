...
class PolicyOptimizer:
    def __init__(self, tester, initial_state, pop_size=5000, generations=50):
        self.tester = tester
        ...
        self.toolbox.register("elite_mutate", self.elite_mutation, indpb=0.3)
    ...
    def elite_mutation(self, individual, indpb):
        ...
        noise = tf.where(
            mask,
            tf.concat([
                tf.random.normal([a_size], 0, 3.0),
                tf.random.normal([b_size], 0, 0.3),
                tf.random.normal([len(individual) - a_size - b_size], 0, 0.03)
            ], axis=0),
            tf.zeros([len(individual)], dtype=tf.float32)
        )
        mutated = ind_tensor + noise
    ...
    def evaluate_policy_batch(self, individuals):
        ...
        # Sample actions from first step for debugging
        actions = self.tester.predict_action(initial_states[:, 1:])  # Corrected to self.tester
        tf.print("Actions (first 5, before clip, first step):", actions[:5])
        ...
        x_cost = tf.reduce_mean(trajectories[:, :, 1]**2, axis=1) * 10.0
    ...
    def optimize(self):
        ...
        hof = tools.HallOfFame(5)  # Top 5 for elite breeding
        ...
        for gen in range(self.generations):
            ...
            for mutant in offspring:
                if tf.random.uniform([]) < 0.7:  # mutpb=0.7
                    self.toolbox.mutate(mutant)
                    del mutant.fitness.values

            # Elite breeding session (top 5 from HoF)
            elite_offspring = list(map(self.toolbox.clone, hof.items[:5]))  # Clone top 5
            # Crossover among elites
            for elite1, elite2 in zip(elite_offspring[::2], elite_offspring[1::2]):
                if tf.random.uniform([]) < 0.7:  # cxpb=0.7
                    self.toolbox.mate(elite1, elite2)
                    del elite1.fitness.values
                    del elite2.fitness.values
            # Subtle mutation for elites
            ...
            combined = population + offspring + elite_offspring
            population[:] = self.toolbox.select_best(combined, self.pop_size)  # Keep best

            # Diversity check: Reintroduce random individuals if all fitnesses are identical
            fits = [ind.fitness.values[0] for ind in population]
            if max(fits) == min(fits):
                tf.print("WARNING: Population converged to single fitness, reintroducing diversity")
                random_ind = self.toolbox.population(n=self.pop_size // 10)  # 10% new random
                population[-len(random_ind):] = random_ind  # Replace worst 10%
