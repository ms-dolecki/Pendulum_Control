...
class KinematicSystem:
    def __init__(self, a=1.0, b=0.5, c=0.5, d=np.sqrt(3), max_frames=10000):
...
if __name__ == "__main__":
    ...
    alphas_end = np.array([np.pi / 4, np.pi / 2, np.pi / 2])
    
    # Add initial transition
    system.add_frames(alphas_start, alphas_end, delta_T=2.0)
    
    # Show the plot non-blocking
    system.show()
    
    # Add more frames (runs immediately after initial pause)
    import time
    time.sleep(3)  # Wait for first animation to finish and pause
    system.add_frames(alphas_end, alphas_start, delta_T=1.0)
    system.add_frames(alphas_start, alphas_end, delta_T=0.2)
    system.add_frames(alphas_end, alphas_start, delta_T=0.1)
    system.add_frames(alphas_start, alphas_end, delta_T=0.2)
    
    # Keep script alive to allow further interaction
