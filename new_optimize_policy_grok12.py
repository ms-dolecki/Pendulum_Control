...
class PolicyOptimizer:
    ...
    def compute_gradient_batch_tf(self, base_params):
        ...
        batch_size = 100  # Process 100 perturbations at a time
        ...
        num_batches = (num_elites * num_params + batch_size - 1) // batch_size  # Ceiling division
        fitness_plus = tf.TensorArray(dtype=tf.float32, size=num_batches)
        fitness_minus = tf.TensorArray(dtype=tf.float32, size=num_batches)

        for i in tf.range(num_batches):
            start_idx = i * batch_size
            end_idx = tf.minimum(start_idx + batch_size, num_elites * num_params)
            batch_indices = indices_plus[start_idx:end_idx]
            batch_updates_plus = tf.fill([tf.shape(batch_indices)[0]], epsilon)
            batch_updates_minus = tf.fill([tf.shape(batch_indices)[0]], -epsilon)
            
            batch_perturbed_plus = tiled_params + tf.scatter_nd(batch_indices, batch_updates_plus, tf.shape(tiled_params))
            batch_perturbed_minus = tiled_params + tf.scatter_nd(batch_indices, batch_updates_minus, tf.shape(tiled_params))
            
            tf.print(f"Batch {i}: perturbed_plus shape:", tf.shape(batch_perturbed_plus))
            
            batch_fitness_plus = self.evaluate_policy_batch_tf(batch_perturbed_plus)
            batch_fitness_minus = self.evaluate_policy_batch_tf(batch_perturbed_minus)
            
            fitness_plus = fitness_plus.write(i, batch_fitness_plus)
            fitness_minus = fitness_minus.write(i, batch_fitness_minus)

        fitness_plus = tf.concat(fitness_plus.stack(), axis=0)
        fitness_minus = tf.concat(fitness_minus.stack(), axis=0)
        ...
        return gradients, base_fitness, tiled_params, fitness_plus, tiled_params, fitness_minus
    ...
    def gradient_descent_batch(self, elites, learning_rate=0.1):
        tf.print("gradient_descent_batch - elites length:", len(elites))
        elites_tensor = tf.constant([ind[:] for ind in elites], dtype=tf.float32)
        tf.print("gradient_descent_batch - elites_tensor shape:", tf.shape(elites_tensor))
    ...
    def optimize(self, load_checkpoint=None):
        os.makedirs('checkpoints', exist_ok=True)

        if load_checkpoint is not None:
            checkpoint_file = f'checkpoints/population_gen_{load_checkpoint}.pkl'
            x_data_file = f'checkpoints/X_data_gen_{load_checkpoint}.npy'
            y_data_file = f'checkpoints/y_data_gen_{load_checkpoint}.npy'
            if os.path.exists(checkpoint_file) and os.path.exists(x_data_file) and os.path.exists(y_data_file):
                with open(checkpoint_file, 'rb') as f:
                    population = pickle.load(f)
                self.tester.X_data = tf.constant(np.load(x_data_file), dtype=tf.float32)
                self.tester.y_data = tf.constant(np.load(y_data_file), dtype=tf.float32)
                start_gen = load_checkpoint + 1
                tf.print(f"Loaded checkpoint from gen {load_checkpoint}: population size {len(population)}, "
                         f"X_data shape {tf.shape(self.tester.X_data)}, y_data shape {tf.shape(self.tester.y_data)}")
            else:
                raise FileNotFoundError(f"Checkpoint files for gen {load_checkpoint} not found")
        else:
        ...
        if load_checkpoint is None:
            ...
            fitnesses = self.evaluate_policy_batch(population)
            tf.print("Loaded population fitnesses (min, max, mean):", 
                     tf.reduce_min(fitnesses), tf.reduce_max(fitnesses), tf.reduce_mean(fitnesses))
            for ind, fit in zip(population, fitnesses):
                ind.fitness.values = (float(fit.numpy()),)
            hof.update(population)
            tf.print("HoF size after load:", len(hof.items))

        for gen in range(start_gen, self.generations):
            elites = list(map(self.toolbox.clone, hof.items[:5]))
            tf.print("Gen", gen, "elites length:", len(elites))
            ...
            checkpoint_file = f'checkpoints/population_gen_{gen}.pkl'
            x_data_file = f'checkpoints/X_data_gen_{gen}.npy'
            y_data_file = f'checkpoints/y_data_gen_{gen}.npy'
            with open(checkpoint_file, 'wb') as f:
                pickle.dump(population, f)
            np.save(x_data_file, self.tester.X_data.numpy())
            np.save(y_data_file, self.tester.y_data.numpy())
...
def main():
    ...
    best_policy = optimizer.optimize(load_checkpoint=0)
