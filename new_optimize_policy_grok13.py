...
# Enable GPU (remove CPU-only restriction)
gpus = tf.config.list_physical_devices('GPU')
if gpus:
    tf.config.set_visible_devices(gpus[0], 'GPU')
    tf.config.experimental.set_memory_growth(gpus[0], True)
    print("GPU activated:", gpus[0])
else:
...
class PolicyOptimizer:
    ...
    def optimize(self, checkpoint_gen=None):
        os.makedirs('checkpoints', exist_ok=True)

        if checkpoint_gen is not None:
            checkpoint_file = f'checkpoints/population_gen_{checkpoint_gen}.pkl'
            x_data_file = f'checkpoints/X_data_gen_{checkpoint_gen}.npy'
            y_data_file = f'checkpoints/y_data_gen_{checkpoint_gen}.npy'
            if os.path.exists(checkpoint_file) and os.path.exists(x_data_file) and os.path.exists(y_data_file):
                ...
                start_gen = checkpoint_gen + 1
                tf.print(f"Loaded checkpoint from gen {checkpoint_gen}")
            else:
                raise FileNotFoundError(f"Checkpoint files for gen {checkpoint_gen} not found")
        else:
        ...
        for gen in range(start_gen, self.generations):
            ...
            tf.print(f"Saved checkpoint for gen {gen}")
...
def main():
    ...
    optimizer = PolicyOptimizer(tester, initial_state, pop_size=1000, generations=50)
    best_policy = optimizer.optimize()  # Add checkpoint_gen=5 to resume from gen 5
