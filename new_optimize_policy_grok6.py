...
class PendulumGPTester:
    def __init__(self, num_links=1, sim_time=10.0, dt=0.0125):
        ...
        self.y_std = None
        self.mask = None
        self.X_data = None  # Class-level storage for all X data
        self.y_data = None  # Class-level storage for all y data
    ...
    def run_simulation_with_policy(self, policy_params, initial_state, idx, gen):
        """Run simulation with a specific policy and save trajectory."""
    ...
    def fit_gp(self):  # No args, uses class-level X_data and y_data
        subset_size = min(1000, self.X_data.shape[0])
...
class PolicyOptimizer:
    ...
    def optimize(self):
        ...
        for gen in range(self.generations):
            ...
            policy_params = (a, b, c)
            self.tester.run_simulation_with_policy(policy_params, self.initial_state, idx=0, gen=gen)  # Data added to self.X_data, self.y_data
            ...
            if gen % 5 == 0 and gen > 0:
                ...
                # Reevaluate HoF with updated GP
                hof_fitnesses = self.evaluate_policy_batch(hof.items)
                for ind, fit in zip(hof.items, hof_fitnesses):
                    ind.fitness.values = (float(fit.numpy()),)
                hof.update(hof.items)
                tf.print(f"Gen {gen} HoF fitnesses after GP update:", [ind.fitness.values[0] for ind in hof.items])
...
def main():
    ...
    tester.fit_gp()  # Initial fit with 20 random sims
