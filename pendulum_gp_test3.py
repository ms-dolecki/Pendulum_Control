...
class PendulumGPTester:
    ...
    def predict_action(self, states):
        ...
        return tf.clip_by_value(actions, -5.0, 5.0)  # Shape: [batch_size]
    ...
    def fit_gp(self, X_data, y_data):
        subset_size = min(1000, X_data.shape[0])  # Changed to 1000 for quicker testing
        ...
        self.mask = tf.where(mask)[:, 0]  # Tensor of indices, e.g., [1, 2, 3, 4, 5, 6]
    ...
    def predict_trajectory(self, initial_states):
        ...
        def body(i, prev_states, states_ta):
            ...
            mean = self.model.predict_f(X_normalized)[0]  # Shape: [batch_size, 5] - Extract mean from tuple
            next_states = mean * self.y_std + self.y_mean  # Shape: [batch_size, 5]
            states_new = tf.concat([time_steps[i:i+1, tf.newaxis] * tf.ones([batch_size, 1]), next_states], axis=1)  # Shape: [batch_size, 6]
            x = states_new[:, 1]
            x_dot = states_new[:, 2]
            states_new = tf.where(tf.abs(x)[:, tf.newaxis] > 3, 
                                  tf.concat([states_new[:, :2], tf.zeros([batch_size, 1]), states_new[:, 3:]], axis=1), 
                                  states_new)
            states_new = tf.where(tf.abs(x)[:, tf.newaxis] > 3, 
                                  tf.concat([states_new[:, :1], tf.clip_by_value(states_new[:, 1:2], -3, 3), states_new[:, 2:]], axis=1), 
                                  states_new)
            states_new = tf.ensure_shape(states_new, [None, 6])  # Dynamic batch size, fixed 6
...
def main():
    ...
    # Generate 3 initial states
    initial_states = np.stack([tester.generate_random_initial_state() for _ in range(3)], axis=0)  # Shape: (3, 6)
    print(f"Initial states:\n{initial_states}")
    start_time = time.time()
    trajectories = tester.predict_trajectory(initial_states)
    print(f"Prediction took {time.time() - start_time:.2f}s")

    # Save 3 trajectories to separate CSV files
    trajectories = tf.transpose(trajectories, [1, 0, 2])  # Shape: [3, steps, 6]
    for i, trajectory in enumerate(trajectories):
        with open(f'policies/predicted_trajectory_{i}.csv', 'w', newline='') as f:
            writer = csv.writer(f)
